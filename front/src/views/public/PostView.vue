...
const loadData = async () => {
  ...
  try {
    ...
    loadError.value = getApiErrorMessage(error, 'Failed to load post.')
  } finally {
    loading.value = false
  }
}
...
const handleLike = async (positive: boolean) => {
  ...
  try {
    ...
    notify(getApiErrorMessage(error, 'An error occurred. Please try again.'), 'info')
  }
}
...
<template>
  <div v-if="loading" class="max-w-3xl mx-auto py-20 px-4 font-mono text-sm text-zinc-500">
    Loading post...
  </div>
  <div v-else-if="loadError" class="max-w-3xl mx-auto py-20 px-4">
    <p class="font-mono text-sm text-red-600 dark:text-red-400 mb-4">{{ loadError }}</p>
    <button
        @click="loadData"
        class="px-4 py-2 border border-zinc-300 dark:border-zinc-700 text-xs font-bold uppercase tracking-wider hover:bg-zinc-50 dark:hover:bg-zinc-900"
    >
      Retry
    </button>
  </div>
  <article v-else-if="post" class="max-w-3xl mx-auto py-12 px-4 md:px-6">
